    ...
    <div v-show="isMasterFilterActive">
      ...
      <div style="display: flex; justify-content: flex-end;">
        <form @submit.prevent="filter" v-show="isFilterActive">
          <div>
            <input type="checkbox" id="AmisFilters" value="amis" v-model="internalFilters" />
            <label for="AmisFilters" id="checkout">Amis</label>
            <input type="checkbox" id="groupesFilters" value="groupes" v-model="internalFilters" />
            <label for="groupesFilters" id="checkout">Groupes</label>
            <input type="checkbox" id="groupesAdminFilters" value="groupesAdmin" v-model="internalFilters" />
            <label for="groupesAdminFilters" id="checkout">Groupes dont on est l’administrateur</label>
          </div>
        </form>
        <svg @click="toggleFiltre" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
          fill="none" stroke="rgba(225, 237, 222)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div v-show="isSearchActive">
          ...
          <div class="dropdown-content" v-show="searchText.length > 0 && filteredOptions.length > 0">
            <div v-for="option in filteredOptions" :key="option.id" style="margin-top: 20px;">
              ...
              <label :for="option.id"  id="checkout">{{ option.name }}</label>
            </div>
          </div>
        </div>
        <svg @click="toggleSearch" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
          fill="none" stroke="rgba(225, 237, 222)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
@media screen and (max-width: 499px) {
  label[for="groupesAdminFilters"] {
    display: block;
    margin-top: 10px; /* Ajoutez une marge si nécessaire */
  }
}
</style>
